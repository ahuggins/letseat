    ...
    return (
        <>
            ...
            >
                ...
                >
                    <div className={`rounded-xl border border-red-200 bg-white shadow-lg ` + contentClasses}>{children}</div>
                </div>
            </Transition>
        </>
    );
...
const DropdownLink = ({ className = '', children, ...props }: InertiaLinkProps) => {
    return (
        <Link
            {...props}
            className={
                'block w-full px-4 py-2 text-start text-sm leading-5 text-zinc-700 hover:bg-red-100 hover:text-red-900 focus:outline-none focus:bg-red-100 transition duration-150 ease-in-out ' +
                className
            }
        >
            {children}
